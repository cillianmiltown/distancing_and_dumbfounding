...
pwr::pwr.chisq.test(df=(chi_df),N=4800,sig.level = .05, power=.9)$w*(sqrt(df))

df <- 2
small <- .1/sqrt(df)
medium <- .3/sqrt(df)
large <- .5/sqrt(df)
analysis1 <- c(pwr::pwr.chisq.test(w=small,df=(df),sig.level = .05, power=.9)$N
               ,pwr::pwr.chisq.test(w=medium,df=(df),sig.level = .05, power=.9)$N
               ,pwr::pwr.chisq.test(w=large,df=(df),sig.level = .05, power=.9)$N)
df <- 4
small <- .1/sqrt(df)
medium <- .3/sqrt(df)
large <- .5/sqrt(df)
analysis2 <- c(pwr::pwr.chisq.test(w=small,df=(df),sig.level = .05, power=.9)$N
               ,pwr::pwr.chisq.test(w=medium,df=(df),sig.level = .05, power=.9)$N
               ,pwr::pwr.chisq.test(w=large,df=(df),sig.level = .05, power=.9)$N)
df <- 10
small <- .1/sqrt(df)
medium <- .3/sqrt(df)
large <- .5/sqrt(df)
analysis3 <- c(pwr::pwr.chisq.test(w=small,df=(df),sig.level = .05, power=.9)$N
               ,pwr::pwr.chisq.test(w=medium,df=(df),sig.level = .05, power=.9)$N
               ,pwr::pwr.chisq.test(w=large,df=(df),sig.level = .05, power=.9)$N)

power_table <- 
  `rownames<-`(
    `colnames<-`(rbind(analysis1,analysis2,analysis3), c("Small","Medium","Large")),
